...
step_lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=5, eta_min=0.0001)
...
torch.save(model.state_dict(), f'models/resnet50_cifar10_test_H.pth')
print(f"Finished training the model. Model has been saved")

test_loss, test_acc = hf.test(model, testloader, len(testset), loss_function, "test_H" ,device)
